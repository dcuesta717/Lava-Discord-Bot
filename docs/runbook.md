...
- New girl: send her the server invite → once she's in, `/model add name: user: instagram: tiktok:` (or just tell the bot in chat) → read her `profile.md` in GitHub → 10 min with her on `voice/voice.md` → `/model lanes` if the folders the bot picked are off. Leaving: `/model remove slug: confirm:True` (deletes her channels, role and GitHub folder; data stays).

...
- Away: `/away mode:on` when you're off for the day; check `#girls-questions`; fix answers in `knowledge/faq.md`
- Moments: `/event list` to see what's coming; `/event add` for a one-off (a trend, a local thing); ideas go to each girl automatically
...
- New girl: send her the server invite → once she's in, tell the bot in #bot-dev ("@Lava Bot onboarding Jane, @jane, IG janedoe") or `/model add` → her report + voice draft appear in **#new-girl-reviews** → ✅ Approve, ✏️ Notes (plain English, the bot fixes the files), or 🔁 Redo. Every Sunday 6 AM the bot re-researches everyone (profile, captions, sourcing; the approved voice is never touched). Leaving: `/model remove slug: confirm:True` (deletes her channels, role and files; data stays).

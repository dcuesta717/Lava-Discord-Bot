...
| `models` | implemented | **Onboarding from Discord**: `/model add` or chat → her private channels + deep research (Apify profile + ~65 posts → Claude) → `models/<slug>/` committed to GitHub → live after the redeploy → her **report + voice draft posted in #new-girl-reviews with ✅ Approve / ✏️ Notes / 🔁 Redo** (owners never touch GitHub). Instant kickoff (reels, picks, numbers) when she goes live; weekly re-research; `/model refresh|lanes|list|remove`. |
